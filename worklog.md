...
---
Task ID: review-round-11 (cron webDevReview, trace 1a0db210549216c5-web-cron-review-202609261445)
Agent: main agent (Z.ai Code)
Task: Assess status, QA via agent-browser, implement data-retention policy + per-service alert webhooks (round-10's top two recommendations), terminal-service watch-scope fix, styling polish

Work Log:
- HEALTH CHECK: dev server 200, terminal-service alive, 5/6 services running (nova failed = real), all ingress 200. QA sweep zero console errors, no mobile overflow.
- QA METHOD BUG FOUND IN MY OWN SWEEP: the app is tab-based at "/" only — /activity, /usage etc. are 404 pages (which have no console errors, so a URL-based sweep looks "clean" while testing nothing). Redid the entire sweep by CLICKING through the sidebar nav (10 views) + service detail tabs (8 tabs) + mobile 375px on 3 key views: all clean, no overflow. Future rounds must click through nav, not hit /<view> URLs.
- VLM screenshot review flagged 5 candidates; DOM-measured 4 as false alarms (zero clipped text, all 6 "Logs & Config" buttons at right:1191, editor inputs exactly 299px each, buttons centered against title block). 1 legitimate: failed-state red text too dim under opacity-55 → bumped to full text-red-400.
- MOTIVATION (measured): LogEntry table at 13,822 rows after ~1.7 days (~8-14k rows/day growth, 4.22MB SQLite) — unbounded growth confirmed.
- NEW FEATURE — Data retention policy (round-10 #1 recommendation):
  * src/lib/hoster/retention.ts (new): RetentionConfig {logRetentionDays=7, webhookDeliveryRetentionDays=30, autoPrune=true, pruneIntervalMin=10}, validation (days 1-365, interval 1-1440min), DB>env>default priority (PlatformSetting 'log-retention'; NX_RETENTION_LOG_DAYS/_WEBHOOK_DAYS/_PRUNE_INTERVAL_MIN/_AUTO_PRUNE env fallbacks), 15s config cache.
  * Real prune: pruneNow() deletes LogEntry/WebhookDelivery rows past cutoffs; maybePrune() hooked into the 15s sampler (self-throttled per interval, never throws); manualPrune() for the API. Receipts: every prune that deletes rows writes a LogEntry (source 'retention') — the policy proves itself in the stream it governs.
  * getRetentionStats(): real row counts, oldest-row age, rows-in-last-24h, growth-rate estimate, SQLite file size on disk, last-prune + next-auto-prune countdown.
  * /api/settings/retention (new): GET (config+source+stats) / PUT (validated, 422 with reasons) / DELETE (reset) / POST (prune now).
  * useHoster.ts: useRetentionConfig, useSaveRetentionConfig, useResetRetentionConfig, usePruneNow.
  * ActivityFeedView: new "Data retention" card — 6 live stat tiles (log rows, webhook rows, oldest age, growth/day, db file, next auto-prune), Prune now button (toast with real counts), collapsible editor (3 day/interval fields + auto-prune toggle, keyed remount pattern), config-source chip, methodology footer documenting DB>env>default priority.
  * E2E VERIFIED: GET defaults + real stats; two 422 validation rejections (0 days, 400 days); PUT → source 'db'; manual prune with 5d retention deleted 0 (correct); REAL prune with 1d retention deleted exactly 18 rows (pre-count predicted 18) in 10ms, receipt persisted; DELETE → source 'default'; auto-prune pass confirmed running via sampler (lastPrune set, nextAutoPruneInMs counting down). UI: editor opens with effective values, save (14d) → API source 'db' + "custom config" chip + "log rows > 14d" line, UI reset → source 'default'. Mobile 375px clean, zero console errors.
- NEW FEATURE — Per-service alert webhook targets (round-10 #2 recommendation):
  * Prisma: Service.alertWebhookUrl String @default("") (db:push + standard full-tree restart via restart-dev.py — the known stale-Prisma-client procedure).
  * PATCH /api/services/<id> accepts alertWebhookUrl (http(s) validated, empty clears, 400 with reason on bad input); change receipts logged (source 'settings'); types.ts + serializeService round-trip the field.
  * usage-alerts.ts: fireAlert takes the service's webhook; fanOutAlert now supports an override target {url, minLevel:'any', target:'service'} — the per-service target receives ALL alert levels for THAT service (owner opted in explicitly), independent of the platform-wide webhook's min-level filter. Receipts distinguish "service target" vs "platform target" delivered/failed. SEMANTIC BUG CAUGHT DURING DEV: initial implementation reused 'none' as "no filter" which collides with platform 'none'=disabled — fixed with explicit 'any' sentinel before it could silently skip per-service fan-out.
  * ServiceDetailView webhooks tab: new "Usage alert webhook (this service)" card — armed/platform-only state chip, URL input + save, honest methodology line.
  * E2E VERIFIED (two daemonized test listeners, 9876 platform / 9877 service): fresh 2-step cost ladder [0.005, 5.0] (0.003 already fired today by a prior session — restart-safe dedupe correctly blocked refire, exactly as designed) → warn-level alert fired with real metered data ($0.085 from 3.56 instance-hours) → service listener (9877) received the JSON POST with target:'service' + real numbers, platform listener (9876) received NOTHING (warn < error min-level — level filter works independently per target), LogEntry receipt "service target delivered warn alert → 200 OK". Invalid ftp:// URL rejected 400. UI: card renders, save via UI → DB persisted + armed chip, clear via UI → DB ''. Cleanup: config reset to defaults, service target cleared, listeners killed.
- FIX — terminal-service watch scope (round-10 known risk): moved index.mjs → src/index.mjs; dev script now "node --watch --watch-path=./src src/index.mjs" so node_modules churn can't trigger spurious restarts that kill live PTY sessions. VERIFIED: killed old tree, relaunched via launch.py (ABSOLUTE path — the round-8 lesson), socket.io handshake OK direct :3031 AND via gateway :81 (real sid returned), touched node_modules/ms + node_modules/socket.io → server PID unchanged (13139) = fix works.
- STYLING: (1) failed/stopped service cards — metrics block dims to opacity-55 with honest state text ("process down — see logs" for failed in red, "stopped — no process" for stopped, "no traffic — down" for traffic) instead of uniform "n/a — no process" taking bold visual weight (VLM feedback); (2) placeholder:text-zinc-600 → zinc-500 across 12 inputs in 8 components (contrast); (3) new retention + alert-webhook cards follow the established mono-label/zinc/cyan design language.
- README updated: feature table (+Data retention row, budget-alerts row mentions per-service targets), env table (+4 NX_RETENTION_* vars), API reference (+/api/settings/retention), roadmap refreshed (shipped vs remaining).
- FINAL REGRESSION: tsc clean (src), eslint clean, no recent dev.log errors, all 5 running services' ingress 200, full nav-click sweep of 10 views + 8 service tabs zero console errors, mobile 375px no overflow on services/activity/usage.

Stage Summary:
- Round-10's top two recommendations shipped and E2E-verified: the control-plane DB is now bounded by an operator-editable retention policy with real DELETE passes + receipts, and alert fan-out is two-tier (platform-wide + per-service targets with independent level rules).
- The terminal-service no longer restarts on node_modules churn (live PTY sessions survive package installs).
- QA process hardened: view sweeps must click through the sidebar (URL-based sweeps test 404s — corrected this round).
- Known risks: retention stats' growth/day estimate uses the last-24h window (honest, labeled "~"); the per-service target receives ALL levels by design (documented in the UI — operators who want filtering can point at a filter proxy); alert history for fired thresholds is per-UTC-day (dedupe markers) — a new day refires ladders as thresholds are crossed again (by design, pacing signals).
- Recommended next phase: TLS via Caddy on a real host (still sandbox-blocked), per-service retention overrides, alert fan-out retry with backoff, uv-everywhere cold-deploy caching, an automated repo-sync script (this round repeats the manual push).
